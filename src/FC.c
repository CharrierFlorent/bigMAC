...
int choix_variable (CSP *probleme, HEURISTIQUE heuristique, int *status, int nb_var_instancie)
{
	...
	{
		case MIN_DOMAINE:
			min = probleme->Domain->max_domain+10;
...
int FC(CSP * csp, int * solution, int * var_status, int profondeur, struct timeval * st, struct timeval * et){
			...
			if (csp->Domain->domain_matrix[variable_courante][valeur_courante] == 1){
				noeud_FC++;
             	if (check_forward(csp, var_status, variable_courante, valeur_courante, profondeur)){
					...
					int result = FC(csp, solution, var_status, profondeur+1, st, et);
					if(result == 1)
						return 1;	
					if(result == -1)
						return -1;
