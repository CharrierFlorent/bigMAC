...
CSP * generer_probleme(){
    ...
    while(nb_contrainte != m){
        ...
        if(!csp->constraint_matrix[var1][var2]){
            csp->constraint_matrix[var1][var2] = init_constraint(taille_domaine);
            csp->constraint_matrix[var2][var1] = init_constraint(taille_domaine);

            while(nb_relations != cardinal_R){
                do{
                    d1 = rand()%taille_domaine;
                    d2 = rand()%taille_domaine;
                }while(csp->constraint_matrix[var1][var2]->relations[d1][d2] == 1);
                csp->constraint_matrix[var1][var2]->relations[d1][d2] = 1;
                csp->constraint_matrix[var2][var1]->relations[d2][d1] = 1;
                nb_relations++;
            }
            nb_relations = 0;
            nb_contrainte++;
